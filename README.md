...
Four chat apps, one bot, one brain. Diagrams live in `docs/`:

| File | Shows |
|------|-------|
| [`docs/architecture.png`](docs/architecture.png) | The whole system across channels |
| [`docs/workflow.png`](docs/workflow.png) | A single message, start to finish |
| [`docs/handbook.pdf`](docs/handbook.pdf) | Short handbook for the team |

Both diagrams have editable `.pptx` versions beside them, regenerated by
`docs/make_architecture.py` and `docs/make_slide.py`. They predate Google
...
| `docs/` | Diagrams, handbook, and the scripts that generate them |
